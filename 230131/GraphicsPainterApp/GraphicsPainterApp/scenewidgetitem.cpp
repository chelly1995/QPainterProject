...
SceneWidgetItem::SceneWidgetItem(QWidget *parent) : QWidget(parent)
{
    ...
    connect(combobox, SIGNAL(currentTextChanged(QString)), label, SLOT(setText(QString)));
    ...
    rotateAction->setShortcut(tr("Ctrl+N"));
    rotateAction->setToolTip("Rotate");
    rotateAction->setStatusTip("Rotate");
    connect(rotateAction, &QAction::triggered, this, &SceneWidgetItem::rotate);
...
void SceneWidgetItem::rotate()
{
    emit rotateItem();
}

void SceneWidgetItem::contextMenuEvent(QContextMenuEvent *event)
{
